...
export default function ServicesMegaMenu({ subCategories, isOpen, onOpen, onClose }) {
  return (
    <>
      {subCategories && (
        <Transition
        ...
        >
          <div className={`border border-t-0 absolute z-50 w-full p-4 bg-white shadow `}>
            <div className=" container mx-auto flex flex-wrap gap-y-4">
              {subCategories?.map(
                (
                  { name: sub_category_name, path: sub_category_path, services },
                  index
                ) => (
                  <ul key={index} className="w-1/4">
                    ...
                    {services.map(({ name: service_name, path: service_path }, index) => (
                      <li key={index}>
                        <Link
                          onClick={onClose}
