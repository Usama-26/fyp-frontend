...
import ComboboxMultiple from "../Comboboxes/ComboboxMultiple";
import Spinner from "../Spinner";
import { useGigs } from "@/context/GigContext";
import { useRouter } from "next/router";
const ReactQuill = dynamic(() => import("react-quill"), { ssr: false });

export default function Overview({ step }) {
  const { categories } = useServices();
  const { postGig, isLoading, gig } = useGigs();
  ...
  useEffect(() => {
    if (selectedCategory) {
      setSelectedSubCategory(selectedCategory.sub_categories[0]);
    }
  }, [selectedCategory]);

  useEffect(() => {
    if (selectedSubCategory) {
      setSelectedService(selectedSubCategory.services[0]);
    }
  }, [selectedSubCategory]);

  return (
    <div className="mt-4 p-4 rounded-md shadow-custom-md shadow-neutral-300">
      <div className="grid grid-cols-3">
        <div>STEP 1: OVERVIEW</div>
        <div className="col-span-2 max-w-lg">
          {categories && selectedSubCategory && selectedService && (
            <Formik
              initialValues={{
                title: "",
                description: "",
              ...
              {({ values, errors, touched, submitCount, setFieldValue }) => (
                <Form className="space-y-4">
                  ...
                  <div>
                    ...
                    {errors.tags && touched.tags && submitCount > 0 && (
                      <ErrorMessage
                        name="tags"
                        component={"p"}
                        className="field-error__message"
